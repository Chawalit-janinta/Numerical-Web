...
class lagrange extends Component {
    ...
    exam() {
        ...
        if (this.state.size != '') {
            var i = 0;
            axios.get('http://192.168.99.100:5000/lagrange/')
                .then(response => {
    ...
    save() {
        if (this.state.size != '' && this.state.xdata.length > 1 && this.state.ydata.length > 1) {
            ...
            axios.post('http://192.168.99.100:5000/lagrange/add', eq)
    ...
    cal(v) {
        ...
        if (z.size != '' && z.xdata[0] != 'x0' && z.ydata[0] != 'y0') {
            ...
            var L = []
            var i = 0, j = 0, up = 1, low = 1;
            for (i = 0; i < size; i++) {
                for (j = 0; j < size; j++) {
                    if (j != i) {
                        up *= (x[j] - xt);
                        low *= (x[j] - x[i]);
                        console.log("up", x[j], xt);
                        console.log("low", x[j], x[i]);
                    }
                }
                console.log("up_t = ", up, "low_t = ", low);
                L[i] = up / low;
                up = low = 1;
            }
            console.log(L);
            ...
            <Router>
                ...
                <div style={{ width: "100%", height: "650px", float: "left", padding: "50px" }}>
                    <div className="formula-block" style={{ height: "400px", width: "40%", float: "left" }}>
                        <h1 style={{ fontSize: "40px", fontWeight: "400" }}>Lagrange Interpolation</h1><br />
