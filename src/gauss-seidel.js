...
class gauss_seidel extends Component {
    ...
    exam() {
        ...
        if (this.state.size != '') {
            var i = 0;
            axios.get('http://192.168.99.100:5000/gauss-seidel/')
                .then(response => {
    ...
    save() {
        if (this.state.size != '' && this.state.adata.length > 1 && this.state.bdata.length > 1 && this.state.xdata.length > 1) {
            ...
            axios.post('http://192.168.99.100:5000/gauss-seidel/add', eq)
    ...
    cal(event) {
        ...
        if (x.size != '') {
            ...
            for (i = 0; i < n; i++) {
                ...
                XC[i] = parseFloat(x.xdata[i]);
                XD[i] = parseFloat(x.xdata[i]);
                for (j = 0; j < n; j++) {
            ...
            if (detA != 0) {
                ...
                do {
                    for (i = 0; i < n; i++) {
                        ...
                        sum = parseFloat((B[i] - (sum - (A[i][i] * XC[i]))) / A[i][i]);
                        console.log(sum);
                        X[k] = sum;
                        er[k] = Math.abs(sum - XC[i]) / sum;
                        if (er[k] <= 0.00001) {
                            check_er = 1;
                        }
                        console.log("er = ", er[k]);
            ...
            <Router>
                ...
                <div style={{ width: "100%", height: "650px", float: "left", padding: "50px" }}>
                    <div className="formula-block" style={{ height: "600px", width: "35%", float: "left" }}>
                        <h1 style={{ fontSize: "35px", fontWeight: "400" }}>Gauss Seidel Iteration Method</h1><br />
