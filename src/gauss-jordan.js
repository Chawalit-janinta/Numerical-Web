...
class gauss_jordan extends Component {
    ...
    exam() {
        ...
        if (this.state.size != '') {
            var i = 0;
            axios.get('http://192.168.99.100:5000/gauss-jordan/')
                .then(response => {
    ...
    save() {
        if (this.state.size != '' && this.state.adata.length > 1 && this.state.bdata.length > 1) {
            ...
            axios.post('http://192.168.99.100:5000/gauss-jordan/add', eq)
        ...
        if (x.size != '') {
            ...
            if (detA != 0) {
                ...
                for (i = 0; i < n; i++) {
                    ...
                    var ex2 = B[i] / temp;
                    if (ex2 == Infinity || ex2 == -Infinity || ex2 == NaN) {
                        ex2 = 0;
                    }
                ...
                console.log(A, B, X);
                this.setState({ xdata: B, adata: data, bdata: B });
            }
            else {
                var X = [];
                for (i = 0; i < n; i++) {
            ...
            <Router>
                ...
                <div style={{ width: "100%", height: "650px", float: "left", padding: "50px" }}>
                    <div className="formula-block" style={{ height: "600px", width: "35%", float: "left" }}>
                        <h1 style={{ fontSize: "45px", fontWeight: "400" }}>Gauss Jordan</h1><br />
